...
GameBox.prototype.register = function(anObject)
{
    this.aliveObjects.push(anObject);
}
...
GameObject = function( position ,spritePath, frameWidth, frameHeight,framesPerRow, speed)
{
    this.sheet = new SpriteSheet(spritePath,frameWidth,frameHeight);
    this.sprite = new AnimatedSprite(this.sheet, Math.floor( framesPerRow / 2),0, framesPerRow - 1);

    this.body = new Body(position, speed, this);
    ...
    this.SignalTurning = function()
    {
        ...
        {
            this.sprite = new AnimatedSprite(this.sheet, Math.floor( framesPerRow / 2),framesPerRow, (framesPerRow * 2) -1);
        }
    }
...
GameBox.prototype.update = function(timeElapsed)
{
    if (!pause)
    {
      for (var i = this.aliveObjects.length - 1; i >= 0; i--) 
      {
        this.aliveObjects[i].update(timeElapsed);
      }
    }
}
